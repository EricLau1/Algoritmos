...
int main(){
	...
	int num;
	cout << "Informe a chave que sera excluida: ~~> ";
	cin >> num;
	...
	while(inicio < final && num != Indice[meio][0]){
		if(num > Indice[meio][0]){
			inicio = meio + 1;
		}else{
			final = meio - 1;
		}
	...
	j = Indice[meio][1]; // j recebe o endereço fisico localizado no Arquivo de Indice após a busca binaria
	if(num == Dados[j][0]){
		Dados[j][1] = 1; // excluido logicamente
	}
	
	cout << "\nArquivo de Dados Apos Exclusao" << endl;
	cout << "Posicao\tValor\tStatus\t" << endl;
	for(int i = 0; i<= fim; i++){
